...
- **The Postgres tool truncates multi-row results to a two-row preview** before reaching the LLM. Sentinel works around this by aggregating server-side with `string_agg` rather than relying on raw multi-row output &mdash; that's a real constraint of the runtime, not a solved problem.
